...
const addTransaction = async (transaction) => {
  try {
    const mysql = await connectDB();
    const query = `INSERT INTO transaction (customer_id, message, amount, receipt) VALUES ('${transaction.customer_id}', '${transaction.message}', '${transaction.total_payment}', '${transaction.receipt}')`;
    const [rows, fields] = await mysql.execute(query);
    const id = rows.insertId;
    return id;
  } catch (err) {
...
const addTransactionItems = async (transaction_id, transactionItems) => {
  try {
    const mysql = await connectDB();
...
module.exports = { addCustomer, addTransaction, addTransactionItems };
